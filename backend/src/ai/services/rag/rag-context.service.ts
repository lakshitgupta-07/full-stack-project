...
export const buildRAGContext = async (query: string, travelContext: TravelContext): Promise<string> => {
  ...
  const filterResults = (items: any[]) => {
    return items.filter((result) => {
      if (typeof result.score !== "number" || result.score < RAG_SIMILARITY_THRESHOLD) {
        return false;
      }
      ...
      if (destinations) {
        ...
        if (result.metaData?.country) {
          ...
          if (!destLower.includes(countryLower) && !countryLower.includes(destLower)) {
            return false;
          }
        }
        ...
        if (result.metaData?.city) {
          ...
          if (!destLower.includes(cityLower) && !cityLower.includes(destLower)) {
            return false;
          }
        }
      }
  ...
  if (!relevantResults.length && destinations) {
    console.log(`[RAG Auto-Ingest] No results found. Ingesting new travel guide for: ${destinations}`);
    const messages: AIMessage[] = [
      {
        role: "system",
        content: "You are a professional travel writer. Generate a detailed, comprehensive travel guide for the requested city or country. Include highlights, key attractions, weather, culture, food, and logistics/practical tips."
      },
      {
        role: "user",
    ...
    try {
      ...
      if (generatedText && generatedText.trim()) {
        ...
        console.log(`[RAG Auto-Ingest] Successfully ingested new travel guide for: ${destinations}`);
      ...
      console.error("[RAG Auto-Ingest] Failed to auto-generate travel guide:", err);
    }
  }
