...
import {createEmployeeService, getEmployeeService, getEmployeeByIdService, updateEmployeeService, deleteEmployeeService, bulkCreateEmployeesService } from "../services/employee.service.js";
...
export const bulkCreateEmployees = asyncHandler( async (req, res) => {
    const employees = await bulkCreateEmployeesService( req.user._id);
    res.status(201).json(
        new ApiResponse(201, employees, "100 employees created")
    );
});